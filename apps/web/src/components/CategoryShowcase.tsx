...
function PlaceholderProductCard() {
  return (
    <div className="flex flex-col overflow-hidden rounded-card bg-surface shadow-card">
      <div className="aspect-[4/5] w-full animate-pulse bg-surface-muted" />
      <div className="flex flex-col gap-2 p-3">
        <div className="h-2.5 w-16 animate-pulse rounded bg-line" />
...
function ViewAllCard({ href, title }: { href: string; title: string }) {
  return (
    ...
    >
      <div className="relative flex aspect-[4/5] w-full items-center justify-center overflow-hidden rounded-card bg-hero-gradient text-primary-fg transition-transform duration-200 group-hover:scale-[1.02]">
        <div className="flex flex-col items-center gap-3 px-4 text-center">
          <span className="inline-flex h-12 w-12 items-center justify-center rounded-full border border-primary-fg/70">
            <Icon.ArrowRight size={18} strokeWidth={1.75} aria-hidden />
          </span>
    ...
    <AnimateOnScroll>
      <section className="w-full px-4 py-12 md:px-8 lg:px-12">
        <div className="mb-6 flex items-end justify-between gap-4">
          ...
          <Link
            href={href}
            className="bv-enter bv-delay-1 group hidden shrink-0 rounded-full bg-primary px-6 py-2.5 text-xs font-medium uppercase tracking-[0.18em] text-primary-fg transition-[background-color,transform] duration-200 hover:scale-[1.03] hover:bg-primary-hover md:inline-flex items-center gap-1"
          >
