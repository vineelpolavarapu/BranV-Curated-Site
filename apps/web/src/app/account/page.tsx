...
function NavCard({
  ...
  return (
    <Link
      href={href}
      className={`bv-enter${delay ? ` bv-delay-${Math.min(delay, 7)}` : ''} group flex flex-row items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-3.5 sm:p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary hover:shadow-md`}
    >
      <span className="flex items-center gap-3 min-w-0">
        <span className="flex h-10 w-10 sm:h-11 sm:w-11 shrink-0 items-center justify-center rounded-full bg-blue-50 text-primary transition-colors group-hover:bg-primary group-hover:text-white">
          <Icon className="h-5 w-5" />
        </span>
        <span className="flex flex-col text-left min-w-0">
          <span className="font-heading text-xs sm:text-sm font-bold text-slate-800 group-hover:text-primary transition-colors truncate">{label}</span>
          {value !== undefined && (
            <span className="text-[11px] sm:text-xs font-medium text-slate-500">{value} saved</span>
          )}
        </span>
      </span>
      <ChevronRightIcon className="h-4 w-4 shrink-0 text-slate-400 transition-transform group-hover:translate-x-0.5 group-hover:text-primary" />
    </Link>
  );
}
