...
from ..enums import NotificationChannel, NotificationType, OutboxStatus
...
class NotificationOutbox(Base):
    ...
    scheduledFor: Mapped[datetime] = mapped_column(TIMESTAMP(timezone=False), nullable=False, server_default=text('CURRENT_TIMESTAMP'), name='scheduledFor')
    dispatchedAt: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=False), nullable=True, name='dispatchedAt')
    ...
    __table_args__ = (
        Index('ix_notification_outbox_status_scheduledFor', 'status', 'scheduledFor'),
    )
